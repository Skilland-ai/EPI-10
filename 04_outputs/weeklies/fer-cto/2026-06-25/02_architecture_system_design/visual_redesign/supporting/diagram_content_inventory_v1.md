...
| --- | --- | --- | --- |
| Big-picture boundary view: actors, AWS-owned layer, external SaaS and manual/out-of-system scope | Architecture briefing; Blueprint brief; System design module boundaries | Four-column top layout: `Actors + entry`, `AWS Espana / EPI10-owned`, `External SaaS / provider systems`, `Manual / out-of-system boundary` | Diagram 0 simplified overview before the detailed diagrams |
| Actors: cliente final, Aitor/equipo EPI10, Carmen/direction, Raul, Fer and external providers | System design `Actors`; Blueprint brief; Architecture briefing | `Actors + entry` column, `Raul + Fer` review card and provider columns | Overview text; Diagram 0 actor column; Diagram 1 `Actors` subgraph; Diagram 2 participants; QA section |
| Entry point: web/pago with `lead_created` and `payment_success` | System design Flow 1; Blueprint brief; Architecture briefing | `Actors + entry` column `web/pago` card with two event chips and Flow 1 arrow into AWS | Diagram 0 entry subgraph; Diagram 1 web/pago node; Diagram 2 messages `lead_created` and `payment_success`; Diagram 4 entry boundary |
| EPI10 MVP orchestration service in AWS Espana using TypeScript/NestJS | System design Proposed Components; Architecture briefing; ADR-003/ADR-005 | Large `AWS Espana / EPI10-owned runtime` boundary with MVP service and module cards | Diagram 0 AWS subgraph; Diagram 1 central subgraph; Diagram 2 `MVP`; Diagram 4 owned runtime boundary |
| PostgreSQL technical DB | System design PostgreSQL Technical DB; Data Classes; ADR-004 | Inside AWS boundary as `PostgreSQL tecnico` card with allowed-only list and no-raw-data warning | Diagram 0 PostgreSQL node; Diagram 1 DB node; Diagram 2 event ledger/state/retry messages; Diagram 4 data boundary node |
| Healthie portal/API/webhook boundary | System design Healthie Portal; Integration Contracts; ADR-006 | `External SaaS / provider systems` column, yellow gated Healthie card | Diagram 0 SaaS subgraph; Diagram 1 Healthie node; Diagram 2 Healthie webhook/polling messages; Diagram 4 Healthie storage boundary |
| Odoo operational backoffice | System design Odoo Backoffice; Architecture briefing; ADR-007 | `External SaaS / provider systems` column, Odoo card marked operational source of truth for cases/states/tasks | Diagram 0 SaaS subgraph; Diagram 1 Odoo node; Diagram 2 Odoo case/state/task updates; Diagram 3 state surface note; Diagram 4 Odoo boundary |
| EPI10 Informe Final Copilot | System design Copilot boundary; Flow 4; ADR-009 | `External SaaS / provider systems` column, Copilot/LLM card with pseudonymized draft support and human review gate | Diagram 0 SaaS subgraph; Diagram 1 Copilot node; Diagram 2 Flow 4 messages; Diagram 4 pseudonymized package and human review path |
| TellmeGen external/manual boundary | System design Flow 3; Non-goals; ADR-008 | `Manual / out-of-system boundary` column with TellmeGen manual/no API card | Diagram 0 manual boundary; Diagram 1 TellmeGen manual node; Diagram 2 Flow 3 manual notes; Diagram 4 no raw genetic storage rule |
| Flow 1: lead/payment to operational case | System design Flow 1; Architecture briefing first vertical slice | Numbered arrow path `1` from web/pago into AWS and separate bottom Flow 1 card | Diagram 2 section `Flow 1`; Diagram 0/1 high-level event edge |
| Flow 2: portal onboarding to Odoo state | System design Flow 2 | Numbered arrow path `2` from Healthie to AWS to Odoo plus separate bottom Flow 2 card | Diagram 2 section `Flow 2`; Diagram 0/1 Healthie webhook edge |
| Flow 3: TellmeGen external/manual | System design Flow 3; ADR-008 | Numbered manual path `3` to TellmeGen and Odoo milestone plus separate bottom Flow 3 card | Diagram 2 section `Flow 3`; Diagram 0/1 manual dashed edge |
| Flow 4: report production and delivery | System design Flow 4; Copilot boundary; Architecture briefing | Numbered path `4` through Odoo readiness, human report process, Copilot and Healthie/fallback plus separate bottom Flow 4 card | Diagram 2 section `Flow 4`; Diagram 0 high-level report loop; Diagram 4 Copilot/data path |
...
| Idempotency, retries, dead-letter, alerts and manual recovery | System design Idempotency and Retries; Failure Modes; Architecture briefing | Inside AWS: `Core engines`, `Reliability inside AWS`; red arrows to manual recovery/Odoo task | Diagram 0 reliability node; Diagram 2 retry/dead-letter messages; Diagram 4 recovery path with alert/manual recovery |
